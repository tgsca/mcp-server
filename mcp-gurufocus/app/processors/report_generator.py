    ...
    def generate_summary_report(
        ...
        report = {
            ...
            "kernkennzahlen": {
                ...
                "dividend_yield": summary.get("kennzahlen", {}).get("dividend_yield", 0),
                "free_cashflow_yield": summary.get("kennzahlen", {}).get("free_cashflow_yield", 0),
            },
            "bewertung": {
            ...
            "wachstumsaussichten": {
                "umsatz_wachstum_prognose": estimates.get("wachstumsraten", {}).get("revenue", 0),
                "gewinn_wachstum_prognose": estimates.get("wachstumsraten", {}).get("per share eps", 0),
                "eps_prognose_nächstes_jahr": next(iter(estimates.get("jährlich", {}).values()), {}).get("eps", 0),
            },
            "umsatzstruktur": {
        ...
        ttm_segments = segments.get("geschäftsbereiche", {}).get("ttm", {}).get("segmente", {})
        if ttm_segments:
            # Nach Umsatz sortieren und Top-Segmente auswählen
            sorted_segments = sorted(ttm_segments.items(), key=lambda x: x[1].get("umsatz", 0), reverse=True)
        ...
        if regions_annual:
            ...
            sorted_regions = sorted(regions.items(), key=lambda x: x[1].get("umsatz", 0), reverse=True)
