...
class NewsProcessor:
    ...
    @staticmethod
    def process_news_headlines(headlines_data: Union[str, Dict, List, None]) -> List[Dict[str, str]]:
        """
        ...
        if isinstance(headlines_data, str):
            try:
                ...
                return [{"error": f"Fehler bei der Verarbeitung der Nachrichtendaten: {str(e)}"}]
        
        # Wenn keine Schlagzeilen gefunden wurden, Fehler zurückgeben
        if not headlines:
            return [{"error": "Keine Nachrichten gefunden oder Format nicht unterstützt"}]
