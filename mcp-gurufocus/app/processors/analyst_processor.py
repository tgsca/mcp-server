    ...
    def process_analyst_estimates(data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if annual:
            ...
            for i, date in enumerate(dates):
                annual_data[date] = {
                    "umsatz": AnalystProcessor.get_value_at_index(annual, "revenue_estimate", i),
                    "nettogewinn": AnalystProcessor.get_value_at_index(annual, "net_income_estimate", i),
                    "eps": AnalystProcessor.get_value_at_index(annual, "per_share_eps_estimate", i),
                    "dividende": AnalystProcessor.get_value_at_index(annual, "dividend_estimate", i),
                    "roe": AnalystProcessor.get_value_at_index(annual, "roe_estimate", i),
                    "bruttomarge": AnalystProcessor.get_value_at_index(annual, "gross_margin_estimate", i)
                }
        ...
        if quarterly:
            ...
            for i in range(quarters_to_include):
                quarterly_data[dates[i]] = {
                    "umsatz": AnalystProcessor.get_value_at_index(quarterly, "revenue_estimate", i),
                    "nettogewinn": AnalystProcessor.get_value_at_index(quarterly, "net_income_estimate", i),
                    "eps": AnalystProcessor.get_value_at_index(quarterly, "per_share_eps_estimate", i)
                }
            
            result["quartal"] = quarterly_data
        
        # Langfristige Wachstumsraten
        growth_fields = [k for k in annual.keys() if k.startswith("future_") and k.endswith("_growth")]
        for field in growth_fields:
            display_name = field.replace("future_", "").replace("_estimate_growth", "").replace("_", " ")
