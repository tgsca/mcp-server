...
class StockProcessor:
    ...
    def process_stock_summary(data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        result["kennzahlen"] = {
            ...
            "ebitda_marge": StockProcessor.extract_ratio_value(
                ratio_data, "Operating margin (%)"
            ),
            "netto_marge": StockProcessor.extract_ratio_value(
                ratio_data, "Net-margin (%)"
            ),
            ...
            "schulden_zu_ebitda": StockProcessor.extract_ratio_value(
                ratio_data, "Debt-to-Ebitda"
            ),
            "current_ratio": StockProcessor.extract_ratio_value(
                ratio_data, "Current Ratio"
            ),
            "dividend_yield": StockProcessor.extract_ratio_value(
                ratio_data, "Dividend Yield"
            ),
            "free_cashflow_yield": data.get("summary", {})
            .get("company_data", {})
            .get("FCFyield", 0),
        }
