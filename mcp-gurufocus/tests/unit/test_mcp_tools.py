...
class TestMCPToolFunctions:
    ...
    @patch('app.processors.financials_processor.FinancialsProcessor.process_stock_financials')
    ...
    async def test_get_concise_stock_financials_default_currency(self, mock_client):
        ...
        result = await get_concise_stock_financials("AAPL")
    ...
    @patch('app.processors.analyst_processor.AnalystProcessor.process_analyst_estimates')
    async def test_get_concise_analyst_estimates_tool(self, mock_processor, mock_client):
    ...
    async def test_get_concise_analyst_estimates_api_failure(self, mock_client):
        ...
        assert "error" in result
        assert "Konnte keine Analystenschätzungen für INVALID abrufen" in result["error"]
    ...
    async def test_get_concise_segments_data_tool(self, mock_processor, mock_client):
        ...
        mock_client.return_value = {
            "business": {
                "annual": [{"date": "2023", "iPhone": 200000000000}]
            }
        }
        mock_processor.return_value = {
    ...
    async def test_get_concise_segments_data_default_start_date(self, mock_client):
        ...
        result = await get_concise_segments_data("AAPL")
    ...
    async def test_error_propagation_in_tools(self):
        ...
        with patch('app.main.GuruFocusClient.get_stock_summary', new_callable=AsyncMock) as mock_summary, \
             patch('app.main.GuruFocusClient.get_stock_financials', new_callable=AsyncMock) as mock_financials, \
             patch('app.main.GuruFocusClient.get_analyst_estimates', new_callable=AsyncMock) as mock_estimates, \
             patch('app.main.GuruFocusClient.get_segments_data', new_callable=AsyncMock) as mock_segments, \
             patch('app.main.GuruFocusClient.get_news_headlines', new_callable=AsyncMock) as mock_news:
